...
public:
	...
	**/
	 ...
	 */
	// DEPRECATED
	bool CheckAndSerialize(const RLP &_rlp, RLPBaseData &_rbd, SessionBaseData &caBaseData) override;
	// DEPRECATED
	bool CheckAll(const std::string& sNodeId, SessionBaseData &caBaseData) override ;
	// DEPRECATED
	void ConstructHandShakeRLP(RLPStream &_rlp, RLPBaseData &_rbd) override ;
	// DEPRECATED
	void SaveCADataInSession(const std::string nodeId, SessionBaseData &caBaseData) override ;
...
private:
	...
	// DEPRECATED
	bool signNodeInfo(SessionBaseData & caBaseData);
	// DEPRECATED
	bool signCAInfo(std::string seedStr, SessionBaseData & caBaseData);
	bool checkNodeInfo(const std::string& remoteNodeID, const dev::h256& h);
	// DEPRECATED
	bool checkNodeInfo(const std::string& remoteNodeID, SessionBaseData& caBaseData);
	bool checkIP(std::string remoteNodeID, CaInfo &caInfo);
	// DEPRECATED
	bool checkCA(std::string remoteNodeID, SessionBaseData & caBaseData);
