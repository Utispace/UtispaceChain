	...
	namespace eth
	{
		...
		//	virtual void getAllNodeConnInfo(int, std::map<std::string, NodeConnParams> & mNodeConnParams) const{ getAllNodeConnInfoContractAndConf(mNodeConnParams); };
			...
			**/
			...
			virtual bool CheckAndSerialize(const RLP &_rlp, RLPBaseData &_rbd, SessionBaseData &caBaseData) = 0;
			virtual bool CheckAll(const std::string& , SessionBaseData &) {return true;};
			virtual void ConstructHandShakeRLP(RLPStream &_rlp, RLPBaseData &_rbd) = 0;
			virtual void SaveCADataInSession(const std::string, SessionBaseData &) {};
