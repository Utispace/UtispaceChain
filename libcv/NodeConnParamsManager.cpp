...
#include <libp2p/CVSessionData.h>
...
//签名验证的数据全部从两边数据中读取
bool NodeConnParamsManager::signNodeInfo(SessionBaseData & caBaseData)
{
    try {
        h256 hash;
...
bool NodeConnParamsManager::signCAInfo(std::string seedStr, SessionBaseData & caBaseData)
{
    try
    {
...
// DEPRECATED
bool NodeConnParamsManager::checkNodeInfo(const std::string& sEndPointNodeId, SessionBaseData & caBaseData)
{
    bool bRet = false;
...
// DEPRECATED
bool NodeConnParamsManager::checkCA(std::string remoteNodeID, SessionBaseData & caBaseData)
{
    bool caVerify = eth::NodeConnParamsManager::CAVerify;
    if (!caVerify)
    {
...
// DEPRECATED
bool NodeConnParamsManager::CheckAndSerialize(const RLP &_rlp, RLPBaseData &_rbd, SessionBaseData &caBaseData)
{
...
// DEPRECATED
bool NodeConnParamsManager::CheckAll(const std::string& sNodeId,SessionBaseData &caBaseData) {
...
void NodeConnParamsManager::ConstructHandShakeRLP(RLPStream &_rlp, RLPBaseData &_rbd)
{
    ...
    bool ok = signNodeInfo(cvData);
    if (!ok)
    {
        LOG(ERROR) << "signNodeInfo false!!! nodeId:" << m_host->id().hex() ;
        return;
    }
    ok = signCAInfo(seedStr, cvData);
    if (!ok)
    {
    ...
    LOG(INFO) << "ConstructHandShakeRLP, pub256:" << cvData.getPub256() << ",nodeSign:" << cvData.getNodeSign() << ",nodeId:" << m_host->id().hex() ;
}
// DEPRECATED
//save data to session
void NodeConnParamsManager::SaveCADataInSession(const std::string nodeId, SessionBaseData &caBaseData)
{
    if (nodeId.empty())
    {
        return;
    ...
    m_host->saveSessionDataByNodeId(nodeId, caBaseData);
}
