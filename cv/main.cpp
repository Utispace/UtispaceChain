...
#include <libcv/All.h>
#include <libcv/BlockChainSync.h>
...
void version()
{
	cout << "CyberVein version " << dev::Version << "\n";
	cout << "CyberVein network protocol version: " << dev::eth::c_protocolVersion << "\n";
...
int main(int argc, char** argv)
{
	...
	if (argc > 1 && (string(argv[1]) == "wallet" || string(argv[1]) == "account"))
	{
		...
		{
			string ret;
			cout << "please input keystore path:";
			getline(cin, ret);
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		else if (arg == "--gennetworkrlp")
		{
			...
			{
				int m_cryptoMod = 0;
				std::string m_nodekeyPath = "./network.rlp";
				std::string m_datakeyPath = "./datakey";
				...
				int count = 0;
				netData.appendList(count);
				writeFile(m_nodekeyPath, netData.out());
				writeFile(m_nodekeyPath + ".pub", kp.pub().hex());
				exit(0);
			}
			else
			{
		...
		else if ( arg == "--godminer" )
		{
			...
			{
				LOG(ERROR) << "arg error: " << arg << "  option: " << argv[i] << "\n";
				exit(-1);
			}
	...
	{
		LOG(ERROR) << "no config file import..." << "\n";
		exit(-1);
	}
	...
	if (!godminerJSON.empty())
	{
		try
		{
			...
			if ( chainParams.godMinerStart < 1  || chainParams.godMinerEnd <= chainParams.godMinerStart || chainParams.godMinerList.size() < 1)
			{
				LOG(ERROR) << "mgmt config error..." << "\n";
				exit(-1);
			}
			cout << "mgmt config success... " << "godMinerStart=" << chainParams.godMinerStart << ",godMinerEnd=" << chainParams.godMinerEnd << ",godMinerList.size()=" << chainParams.godMinerList.size() << "\n";

		}
		catch (std::exception &e)
		{
			LOG(ERROR) << "mgmt config file format error..." << e.what() << "\n";
			exit(-1);
		}
	}
	...
	if (!genesisJSON.empty())
	{
		...
		{
			LOG(ERROR) << "genesis config file format error..." << "\n";
			return 0;
		}
	}
	else
	{
		LOG(ERROR) << "no genesis config file import..." << "\n";
		return 0;
	}



	cout << EthPurple "---------------cybervein client starting--------------" EthReset << "\n";
	...
	dev::setSSL(chainParams.ssl);
	dev::setCryptoPrivateKeyMod(chainParams.cryptoprivatekeyMod);
...
//				cout << web3.ethereum()->blockQueueStatus() << "\n";
	...
	if (jsonRPCURL > -1 || ipc || jsonRPCSSLURL > -1)
	{
		...
		auto ethFace = new rpc::Eth(*web3.ethereum(), *accountHolder.get(), chainParams.god);
